...
console.log(products)
  ...
  useEffect(() => {
    if (viewMode === 'spreadsheet') {
      ...
      setSpreadsheetData(flatRows);
      setOriginalSpreadsheetData(JSON.parse(JSON.stringify(flatRows)));
    } else {
      setSpreadsheetData([]);
      setOriginalSpreadsheetData([]);
      setFocusedCell(null);
    }
  }, [viewMode, products, catalogTab, selectedBrand, selectedCategory, selectedStockStatus, searchTerm]);

  const isCellDirty = (rowIndex, field) => {
    const origRow = originalSpreadsheetData[rowIndex];
    const currRow = spreadsheetData[rowIndex];
    if (!origRow || !currRow) return false;
    return origRow[field] !== currRow[field];
  };

  const handleCellChange = (rowIndex, field, value) => {
    setSpreadsheetData(prev => prev.map((row, idx) => {
  ...
  const handleCellKeyDown = (e, rowIndex, colIndex) => {
    if (e.key === 'ArrowDown') {
      ...
      if (targetRow < spreadsheetData.length) {
        document.getElementById(`cell-${targetRow}-${colIndex}`)?.focus();
      }
      ...
      if (e.target.selectionEnd === e.target.value.length) {
        ...
        if (targetCol <= 6) {
          e.preventDefault();
          document.getElementById(`cell-${rowIndex}-${targetCol}`)?.focus();
        }
      }
  ...
  const handleBulkSaveSpreadsheet = async () => {
    const modifiedProductIds = new Set();
    spreadsheetData.forEach((row, idx) => {
      const fields = ['sku', 'basePrice', 'offerPrice', 'quantity', 'isActive'];
      const isRowDirty = fields.some(f => isCellDirty(idx, f));
      if (isRowDirty) {
        modifiedProductIds.add(row.productId);
      }
    });
    ...
    setIsBulkUpdating(true);
    try {
      await Promise.all(Array.from(modifiedProductIds).map(async (productId) => {
        const product = products.find(p => p._id === productId);
        if (!product) return;

        const productRows = spreadsheetData.filter(r => r.productId === productId);
        const hasVariants = product.variants && product.variants.length > 0;
        ...
        if (hasVariants) {
          const updatedVariants = product.variants.map(v => {
            const gridRow = productRows.find(r => r.variantId === v._id);
            if (gridRow) {
              return {
                ...v,
                sku: gridRow.sku,
                price: Number(gridRow.basePrice) || 0,
                offerPrice: Number(gridRow.offerPrice) || 0,
                quantity: Number(gridRow.quantity) || 0,
                isActive: gridRow.isActive
              };
            }
            return v;
          });
          formData.append('variants', JSON.stringify(updatedVariants));
          formData.append('isActive', product.isActive !== false);
        } else {
          const gridRow = productRows[0];
          formData.append('eanNumber', gridRow.sku || '');
          formData.append('basePrice', Number(gridRow.basePrice) || 0);
          formData.append('offerPrice', Number(gridRow.offerPrice) || 0);
          formData.append('totalQuantity', Number(gridRow.quantity) || 0);
          formData.append('isActive', gridRow.isActive);
          formData.append('variants', JSON.stringify([]));
        }

        await axios.put(`${BASE_URL}/api/products/${productId}`, formData, { headers });
      }));

      showAlert('Spreadsheet updates saved successfully!');
      fetchData();
    } catch (err) {
      console.error('Failed to save bulk updates', err);
      showAlert(err.response?.data?.message || 'Failed to save spreadsheet updates.');
    } finally {
      setIsBulkUpdating(false);
    }
  };
  ...
  const renderSpreadsheetView = () => {
    return (
      <div key="spreadsheet-view-table-wrapper" className="bg-transparent backdrop-blur-2xl border border-white/10 shadow-2xl shadow-black/50 rounded-2xl md:rounded-3xl overflow-hidden flex flex-col h-full animate-in fade-in duration-200">
        <div className="overflow-auto custom-scrollbar max-h-[70vh]">
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-20 bg-slate-900/95 backdrop-blur-md border-b border-white/10 text-slate-300 text-xs uppercase tracking-wider shadow-md">
              <tr className="align-middle">
                <th className="p-4 w-12 text-center">S.No</th>
                <th className="p-4 min-w-[200px]">Product Name</th>
                <th className="p-4 min-w-[120px]">Variant</th>
                <th className="p-4 min-w-[150px]">SKU / EAN</th>
                <th className="p-4 w-32">Base Price</th>
                <th className="p-4 w-32">Offer Price</th>
                <th className="p-4 w-32">Stock Qty</th>
            ...
            <tbody className="divide-y divide-white/10">
              {spreadsheetData.length > 0 ? (
                spreadsheetData.map((row, index) => {
                  const isRowModified = ['sku', 'basePrice', 'offerPrice', 'quantity', 'isActive'].some(f => isCellDirty(index, f));
                  return (
                    <tr 
                      key={`${row.productId}-${row.variantId || 'main'}-${index}`}
                      className={`hover:bg-white/[0.01] transition-colors ${isRowModified ? 'bg-amber-500/[0.02]' : ''}`}
                    >
                      {/* 1. S.No */}
                      <td className="p-4 text-xs font-semibold text-slate-500 text-center">{index + 1}</td>

                      {/* 2. Product Name */}
                      <td className="p-4 text-xs font-bold text-white truncate max-w-[240px]" title={row.productName}>
                        {row.productName}
                      </td>

                      {/* 3. Variant Name */}
                      <td className="p-4 text-xs text-slate-400 font-semibold truncate max-w-[120px]">
                        {row.variantName || <span className="text-slate-600 italic text-[10px]">None</span>}
                      </td>
                      ...
                      <td className="p-2">
                        <input
                          ...
                          className={`w-full bg-transparent px-2 py-1 text-xs text-white rounded-lg focus:outline-none focus:bg-white/5 border ${
                            isCellDirty(index, 'sku') 
                              ? 'border-amber-500/50 shadow-sm shadow-amber-500/10' 
                              : 'border-transparent hover:border-white/5 focus:border-blue-500/50'
                          }`}
                      ...
                      <td className="p-2">
                        <input
                          ...
                          type="number"
                          value={row.basePrice}
                          onChange={(e) => handleCellChange(index, 'basePrice', e.target.value)}
                          onKeyDown={(e) => handleCellKeyDown(e, index, 4)}
                          className={`w-full bg-transparent px-2 py-1 text-xs text-white rounded-lg focus:outline-none focus:bg-white/5 border ${
                            isCellDirty(index, 'basePrice') 
                              ? 'border-amber-500/50 shadow-sm shadow-amber-500/10' 
                              : 'border-transparent hover:border-white/5 focus:border-blue-500/50'
                          }`}
                      ...
                      <td className="p-2">
                        <input
                          ...
                          onChange={(e) => handleCellChange(index, 'offerPrice', e.target.value)}
                          onKeyDown={(e) => handleCellKeyDown(e, index, 5)}
                          className={`w-full bg-transparent px-2 py-1 text-xs text-white rounded-lg focus:outline-none focus:bg-white/5 border ${
                            isCellDirty(index, 'offerPrice') 
                              ? 'border-amber-500/50 shadow-sm shadow-amber-500/10' 
                              : 'border-transparent hover:border-white/5 focus:border-blue-500/50'
                          }`}
                      ...
                      <td className="p-2 text-center">
                        <button
                          type="button"
                          onClick={() => handleCellChange(index, 'isActive', !row.isActive)}
                ...
                <tr>
                  <td colSpan={8} className="p-12 text-center text-slate-500 italic text-sm">
                    No matching products/variants found.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {/* Sticky Spreadsheet Bottom Save Actions Bar */}
        {spreadsheetData.some((row, idx) => ['sku', 'basePrice', 'offerPrice', 'quantity', 'isActive'].some(f => isCellDirty(idx, f))) && (
          <div className="p-4 border-t border-white/10 bg-slate-950/80 backdrop-blur-md flex justify-between items-center animate-in slide-in-from-bottom duration-200">
            <span className="text-xs text-amber-400 font-semibold flex items-center gap-1.5 animate-pulse">
              <FiAlertCircle /> Unsaved changes in spreadsheet grid
            </span>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => {
                  setSpreadsheetData(JSON.parse(JSON.stringify(originalSpreadsheetData)));
                }}
                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold rounded-xl text-xs transition-colors cursor-pointer border border-white/5"
              >
                Discard Changes
              </button>
              <button
                type="button"
                onClick={handleBulkSaveSpreadsheet}
                disabled={isBulkUpdating}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-xs transition-colors cursor-pointer shadow-lg shadow-blue-500/10 flex items-center gap-2"
              >
                {isBulkUpdating ? (
                  <>
  ...
  return (
    <div className="relative space-y-4 min-h-full z-0">
      ...
      <div className="flex border-b border-white/10 justify-between items-center mb-2 flex-wrap gap-4">
        ...
        {/* View Mode Toggle */}
        <div className="flex bg-white/[0.03] border border-white/10 rounded-xl p-0.5 mb-2.5 shrink-0 select-none">
          <button
            type="button"
